...
file_dir = "D://enrichment_test//GBM//SNFCC//SNFCC_param_comb//Labels//"

save_dir_2 = "D://enrichment_test//GBM//SNFCC//surv_pval.txt"
...
for (j in 1:6){
  for (k in 10:30){
    ...
    resultSNFCC=ExecuteSNF.CC(datasets=datas, clusterNum=classi, K = k, alpha = alpha, t = 20,
                              maxK = 10, pItem = 0.8, reps = 20, title = "ConsensusClusterResult",
                              plot = "png", finalLinkage = "average")
    
    # add group column to survival data
    surv$Labels<-resultSNFCC$group
    print(surv$Labels)
