...
train_model_dir = "./train_model/COAD/partial_train/0.5/"
xlsx_dir = "./partial_model/COAD/0.5/"
...
class ConvAE:
    def __init__(self, n_input, kernel_size, n_hidden, reg_const1=1.0, reg_const2=150.0, batch_size=256, denoise=False):
        ...
        # self.x4 = tf.concat((self.z_conv1, self.z_conv2), 1)
        # print(self.x4.shape)
        ...
        #
        # self.recon_loss = self.recon_loss1 + self.recon_loss2  + self.recon_loss4
        # self.z_zc_loss = self.z_zc_loss1 + self.z_zc_loss2  + self.z_zc_loss4
        # self.reg_loss = self.reg_loss1 + self.reg_loss2 + self.reg_loss4
...
def save_Coef(Coef, i, j):
    print(Coef)
    writer = pd.ExcelWriter(xlsx_dir + "model19/W%d_%d.xlsx" % (i, j))
    C = pd.DataFrame(Coef)
    C.to_excel(writer, 'page_1', float_format='%.8f')  # float_format 控制精度
    writer.save()
    print(xlsx_dir + 'model19/W%d_%d.xlsx' % (i, j))
...
def main():
    ...
    for i in range(20):  # 总共训练多少次
        ...
        file = open(xlsx_dir + 'model19/W%d.txt' % i, 'w')
